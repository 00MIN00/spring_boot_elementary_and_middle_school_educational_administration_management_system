...
<section id="container">
    ...
    <div class="row">
        ...
        <div id="operationPanel"class="col-lg-9 main-chart">
            ...
            <form   action="${pageContext.servletContext.contextPath}/upUpdateStudent"
                ...
                <div class="content-panel" style="margin-top: 100px;">
                    <table class="table table-striped table-advance table-hover">
                        ...
                        <tr style="background-color: rgba(0,0,0,0.27)">
                            ...
                            <th style="padding-left:50px;border: 1px solid rgba(0,0,0,0.28); "><input type="text" name="studentName" value="${userItem.studentName}" /></th>
                            ...
                            <th style="padding-left:50px;border: 1px solid rgba(0,0,0,0.28); ">
                                <select name="sex" class="form-select form-select-lg mb-3" aria-label=".form-select-lg example">
                                    <c:if test="${userItem.sex=='0'}">
                                        <option value="0" selected>女</option>
                                        <option value="1">男</option>
                                    </c:if>
                                    <c:if test="${userItem.sex=='1'}">
                            ...
                            <th rowspan="3" style="border: 1px solid rgba(0,0,0,0.28);">
                                <img id="upimg" src="${pageContext.servletContext.contextPath}/assets/img/user/${userItem.studentIdImg}"><br/>
                                <input id="upimgFile" type="file" name="setfile">
                            </th>
                        </tr>
                        <tr>
                            <th style="padding-left:50px;border: 1px solid rgba(0,0,0,0.28); ">学号：</th>
                            <th style="padding-left:50px;border: 1px solid rgba(0,0,0,0.28); "><input type="text" name="studentId" value="${userItem.studentId}" readonly/></th>
                            <th style="padding-left:50px;border: 1px solid rgba(0,0,0,0.28); ">密码：</th>
                            <th style="padding-left:50px;border: 1px solid rgba(0,0,0,0.28); "><input type="text" name="studentpassword" value="${userItem.studentpassword}" /></th>
                        </tr>
                        <tr style="background-color: rgba(0,0,0,0.27)">
                            <th style="padding-left:50px;border: 1px solid rgba(0,0,0,0.28); ">出生时间：</th>
                            <th style="padding-left:50px;border: 1px solid rgba(0,0,0,0.28); ">
                                <input type="date" id="start" name="birthYear" value="<fmt:formatDate value='${userItem.birthYear}' pattern='yyyy-mm-dd'/>">
                            </th>
                            <th style="padding-left:50px;border: 1px solid rgba(0,0,0,0.28); ">入学时间：</th>
                            <th style="padding-left:50px;border: 1px solid rgba(0,0,0,0.28); "> <fmt:formatDate value="${userItem.enrollmentDate}" pattern="yyyy-mm-dd" />
                            </th>
                        </tr>
                        <tr>
...
<%--                                    <option selected>${userItem.studentClass}</option>--%>
                                    <c:forEach items="${studentClassLits}" var="studentClass">
                                        <c:if test="${userItem.studentClass==studentClass.clbumName}">
                                            <option value="${studentClass.uid}"selected>${studentClass.clbumName}</option>
                                        </c:if>
                                    </c:forEach>
                                    <c:forEach items="${studentClassLits}" var="studentClass">
                                        <c:if test="${userItem.studentClass!=studentClass.clbumName}">
